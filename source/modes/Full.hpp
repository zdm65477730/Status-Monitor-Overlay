		...
		auto Status = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
			...
			if (GameRunning) {
				...
				if (settings.showFPS == true) {
					static auto dimensions = renderer->drawString("FPSTitleFullOverlayCustomDrawerText"_tr.c_str(), false, COMMON_MARGIN + width_offset, 120, 20, renderer->a(0xFFFF));
				...
				if ((settings.showRES == true) && *API_shared == 1) {
					width_offset = 170;
					renderer->drawString("ResolutionTitleFullOverlayCustomDrawerText"_tr.c_str(), false, COMMON_MARGIN + width_offset, 185, 20, renderer->a(0xFFFF));
