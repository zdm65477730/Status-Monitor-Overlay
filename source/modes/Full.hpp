	...
	virtual void update() override {
		...
		if ((settings.showRES == true) && GameRunning && renderCalls_shared) {
			if (!resolutionLookup) {
				...
				if (*API_shared == 1) {
					snprintf(Resolutions_c, sizeof(Resolutions_c), "UpdateResolutionTitleFullOverlayCustomDrawerText"_tr.c_str(), m_resolutionOutput[0].width, m_resolutionOutput[0].height, m_resolutionOutput[1].width, m_resolutionOutput[1].height);
				}
			}
		}
