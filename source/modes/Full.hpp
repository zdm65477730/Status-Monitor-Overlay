...
public:
	...
	virtual void update() override {
		...
		if (settings.showFPS == true) {
			...
			else snprintf(FPS_var_compressed_c, sizeof FPS_var_compressed_c, "UpdateFPSTitleFullOverlayCustomDrawerText"_tr.c_str(), FPS, useOldFPSavg ? FPSavg_old : FPSavg);
		}
		...
		if ((settings.showRES == true) && GameRunning && NxFps) {
			if (!resolutionLookup) {
				...
				if (!m_resolutionOutput[1].width || !m_resolutionOutput[0].width) {
					...
					else snprintf(Resolutions_c, sizeof(Resolutions_c), "UpdateOneResolutionTitleFullOverlayCustomDrawerText"_tr.c_str(), m_resolutionOutput[1].width, m_resolutionOutput[1].height);
				}
			...
			if (settings.showRDSD == true && GameRunning && NxFps) {
				if ((NxFps -> readSpeedPerSecond) != 0.f) snprintf(readSpeed_c, sizeof(readSpeed_c), "UpdateReadSpeedTitleFullOverlayCustomDrawerText"_tr.c_str(), (NxFps -> readSpeedPerSecond) / 1048576.f);
				else snprintf(readSpeed_c, sizeof(readSpeed_c), "UpdateReadSpeedNDTitleFullOverlayCustomDrawerText"_tr.c_str());
			}
		}
