...
public:
    ...
    virtual void update() override {
        ...
        if (m_show_outline) {
            strncat(Cursor, "UpdateShowOutlineCursorMainMenuCursorText"_tr.c_str(), sizeof Cursor - 1);
    ...
    virtual bool handleInput(u64 keysDown, u64 keysHeld, const HidTouchState &touchPos, HidAnalogStickState leftJoyStick, HidAnalogStickState rightJoyStick) override {
        ...
        if ((keysHeld & HidNpadButton_StickL) && (keysHeld & HidNpadButton_StickR)) { // return to pervious view
            FullMode = true;
            tsl::hlp::requestForeground(true);
            TeslaFPS = 60;
            IsFrameBackground = true;
...
public:
    ...
    virtual tsl::elm::Element *createUI() override {
        memset(BookmarkLabels, 0, NUM_bookmark * 20 + MAX_NUM_cheats * 0x41);
        memset(Variables, 0, 672);
        memset(CheatsLabelsStr, 0, MAX_NUM_cheats * 0x41);
        memset(Cursor, 0, NUM_bookmark * 5 + MAX_NUM_cheats * 5 + 1000);
        memset(MultiplierStr, 0, NUM_bookmark * 5 + MAX_NUM_cheats * 0x41);
        auto rootFrame = new tsl::elm::OverlayFrame("", "");
        auto Status = new tsl::elm::CustomDrawer([](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h) {
